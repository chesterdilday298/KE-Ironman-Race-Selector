  ...
  const getRankedRaces = () => {
    ...
    return filtered.sort((a, b) => {
      const calcScore = (race) => {
        ...
        // --- SWIM SCORING (1-10 MULTIPLIER) ---
        // 1/10 strength = 2.0x penalty, 5/10 = 1.0x, 10/10 = 0.5x
        ...
        else if (race.wetsuit === "Doubtful") s -= (50 * swimMultiplier);

        // --- BIKE SCORING (STRENGTH REDUCES PENALTY) ---
        ...
        // --- RUN SCORING (STRENGTH REDUCES PENALTY) ---
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#231F20', color: 'white', fontFamily: 'Inter, sans-serif', padding: '40px 20px' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
        ...
        {step === 1 && (
          <div>
            <h3>Step 1: Athlete Strength Profile</h3>
            <input 
              type="email" value={email} onChange={(e) => setEmail(e.target.value)} 
              placeholder="Enter Email Address" 
              style={{ width: '100%', padding: '15px', borderRadius: '8px', marginBottom: '25px', color: 'black', border: 'none' }} 
            />
            ...
            <button onClick={() => setStep(2)} style={btnStyle}>Next: Race Distance</button>
          </div>
        )}

        {step === 2 && (
          <div>
            <h3>Step 2: Distance Selection</h3>
        ...
        {step === 6 && (
          <div>
            <h3>Step 6: Climate Preference</h3>
            ...
            {getRankedRaces().map((race, index) => (
              <div key={race.name} style={{ backgroundColor: 'white', color: '#231F20', padding: '15px', borderRadius: '12px', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '0.8rem', marginTop: '5px' }}>
                  Swim: {race.wetsuit} | Bike: {race.bike} | Run: {race.run}
                </div>
              </div>
            ))}
            <button onClick={() => { setStep(1); setSelections({distance: '', goal: '', swimStrength: 5, bikeStrength: 5, runStrength: 5, bikeTerrain: '', runTerrain: '', climate: ''}); }} style={btnStyle}>Start Over</button>
          </div>
        )}
      </div>
    </div>
